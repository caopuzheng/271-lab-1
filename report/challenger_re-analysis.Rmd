...
# todo: add the ylabs and xlabs, rename all the columns
stargazer(mod1,
...
In order to test for the significant of Pressure, we use the Likelihood Ratio Test for each explanatory variable. For this test, we stated a Null hypothesis that $\beta_2 = 0$, or Pressure does not add significant explanationary power for the odds ratio of O-ring failure, on top of the Temp in the model. 
...
Considering both test resutls, we conclude that Pressure provide no additional explanability to the model. Excluding it from the model is especially recommended in this case, considering the sample size of 23 observations, the drop in the degree of freedom is 5% when we add one additonal explanationaryv variable. Keeping the model simple and easy to interpret would also help with the further study. 
...
> Intervals are wide for temperatures less than 65F is wide and narrow for temperatures greater than 65F.

//TODO: talk about this: seems wrong? 
> This is expected, because most of the data have temperatures greater than 65F.
> This is because since $\beta_1$ is negative, as 
