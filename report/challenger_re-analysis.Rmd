...
----
The data set used in this analysis was obtained from the 23 pre-Challenger launches of the space shuttle. It contains 5 columns, Flight, Temp, Pressure, O.ring, Number. Flight is a unique identifier representing each flight observed and recorded. Number represents the total number of O-ring in each spacecraft, here each spacecraft contains 6 O-rings. The O.ring variable is a random variable that denotes the number of O-ring failed at each individual launch. Lastly, Temp and Pressure are both numerical values and are the physical condition measurements taken at each launch. 

There are 23 launches in the data set and there are no missing values in any of the features in the data set. Temperature is shown to have 16 distinct values with a range between 53 to 81 and a mean of 69.57. Pressure have 3 distinct values with a range between 50 to 200 and a mean of 152. It is also worth noting that both temperature and pressure records are full integer value. In term of the response variable, number of failed O-rings, the majority of launch has 0 failed O-ring (16 out of 23). For the failed launch, there are 5 observations with 1 failed O-ring and 2 observations with 2 failed O-rings.
 
In the original research, the researchers treated each of the 6 O-rings as independent for each of the 23 launches. In other words, the failure of one O-ring do not cause any of the other O-ring to fail. This assumption is necessary in order to satisfy the independent assumption of using the binomial model. This binomial model also assumes that each of the O-rings has equal probability to be damaged by either temperature or pressure condition. However, when considering each O-ring as independent, the researcher might not account for a fact that a single failed O-ring could cause the other o-ring to fail and the entire launch to fail. While careful evaluation of the potential dependency between each O-ring is necessary, we will continue our analysis closely follow the independence assumption.
...
stargazer(mod1,
  ...
  title = "Table 1: The Estimated Relationship Between O-Rings Failure and Temperature/Pressure",
...
c<-10
OR.mod1 <- round(exp(c*mod1$coefficients), 3)
OR.mod1
```
...
In order to test for the significant of Pressure, we use the Likelihood Ratio Test for each explanatory variable. For this test, we stated a Null hypothesis that $\beta_2 = 0$, or Pressure does not add significant explanatory power for the odds of O-ring failure, on top of the Temp in the model. 
...
```{r Estimate the logistic regression model}
mod2 <- glm(
...
stargazer(mod2, mod3,
  ...
  title = "Table 2: The Estimated Relationship Between O-Rings Failure and Temperature Effects",
  dep.var.caption = "Output Variable: Log Odds of O-Rings Failure",
  dep.var.labels = "",
  column.sep.width = "-8pt"
)
...
```{r Pi vs Temp prediction and CI}
...
curve(
  ...
  ylim = c(0, 1),
  col = "blue",
  sub = expression(pi == frac(
    e^(beta[0] + beta[1] * Temp),
    1 + e^(beta[0] + beta[1] * Temp)
  )), # this writes the formula below the curve
  # TODO: can we above formula floating?
...
```{r Graph Expected number of failures vs Temp}
curve(
  expr = 6 * exp(
    mod.beta[1] + mod.beta[2] * x
  ) / (1 + exp(mod.beta[1] + mod.beta[2] * x)),
  xlim = c(31, max(challenger$Temp)),
  ylim = c(0, 6),
  col = "blue",
...
result.profile_lr
```
> In order to apply the inference procedures, we assume asympotic properties of a large sample size, independence of each O-Ring failure and each failure has equal probability. At 31F, the model estimes 81.8% probability of O-Rings failure, with 95% confidence interval of 14.2% and 99%.
...
```{r Bootstrap Probability Distribution and QQ plots T = 31, echo=FALSE, message=FALSE}
...
```{r Bootstrap Probability Distribution and QQ plots t=53, echo=FALSE, message=FALSE}
# sliced the data on 1 temperature unit
...
stargazer(model.linear,
  ...
  report = ("vc*p"),
  star.cutoffs = c(0.05, 0.01, 0.001),
  title = "Table 3: Linear Effect of O-Rings Failure and Temperature"
)
```
...
While that is all well and good, performing a linear regression in this aspect violates the model identically independent distribution assumption. The given dataset has the response variable O.ring as an integer count from O to 6. And the original research treated each O.ring as an independent observation. Thus, by simply dividing O.ring by the maximum number of O.ring and performing a linear regression, we are sometime bundling more than one failled O-rings together in one observation. In this case, if we wanted to model this properly based on the i.i.d. assumptions, a logistic regression would have to be use.
