...
OR_CI_2_1<-round(1/(exp( c.value*conf.beta[2:4, 1:2, 1] )), 4)

# Odds for j = 3 vs j = 1 (Shelf 3 vs Shelf 1)
OR_3_1<-round(1/exp(c.value*beta.hat3 ), 4)
OR_CI_3_1<-round(1/(exp( c.value*conf.beta[2:4, 1:2, 2]  )), 4)

# Odds for j = 4 vs j = 1 (Shelf 4 vs Shelf 1)
OR_4_1<-round(1/exp(c.value*beta.hat4 ), 4)
OR_CI_4_1<-round(1/(exp( c.value*conf.beta[2:4, 1:2, 3]  )), 4)
...
df <- drinks %>%
   dplyr::select(numall, prel, nrel, age, rosn, state, desired) 
...
p4 <- df %>%
    ggplot(aes(x = rosn, y = ..prop.., group = 1)) + geom_bar(fill = "DarkBlue",
    color = "black") + geom_text(stat = "count", aes(label = ..count..),
    vjust = -1) + xlab("rosn") + ylab("Proportion") + ylim(0,
    1)
p4

# metric variable: nrel, prel, negevent, posevent, age, desired, state
p5 <- df %>%
  ggplot(aes(rosn, nrel)) +
  geom_boxplot(aes(fill = cut_number(nrel,5))) +
  coord_flip() +
  ggtitle("Subjects' negevent by rosn") +
  theme(plot.title = element_text(lineheight=1, face="bold")) +
  ylab("negevent") +
  xlab(" rosn")
...
p5 <- df %>%
  ggplot(aes(rosn, nrel)) +
  geom_point(mapping = aes(x = rosn, y = nrel))
p5
...
```{r}
mod.neg <- glm(formula = numall ~ nrel+prel+I(nrel*prel)+I(nrel^2)+I(prel^2), family = poisson(link = "log"), data = df)
summary(mod.neg)
Anova(mod.neg, test='LR')

mod.desired <- glm(formula = desired ~ nrel+prel+I(nrel*prel)+I(nrel^2)+I(prel^2), data = df)
summary(mod.desired)
...
```{r}
mod.rosn <- glm(formula = rosn ~ nrel+numall+I(nrel*numall)+I(nrel^2)+I(numall^2), , data = df)
summary(mod.rosn)
Anova(mod.rosn, test='LR')
