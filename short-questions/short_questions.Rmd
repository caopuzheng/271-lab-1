...
# TODO: we should generalize it into a function
stand02 <- function(new, x, col_name) {
  (new - min(x)) / (max(x) - min(x))
}
...
# TODO: dynamically load the numbers  
mean_fat <- 0.3476
mean_sodium <- 0.5240
...
# get the standard deviations P161
...
>
> TODO: Relate your interpretations back to the plots constructed for this exercise.
...
#TODO: rationalize why we are filtering for one day
...
# thoughts: both negative and positive - offsets?
drinks %>%
  ggplot(aes(x=nrel, y= prel, color = numall)) +
  geom_point() +
  coord_quickmap()+
  ggtitle("positive ")
...
#TODO: ask what the question really wants us to model

mod_desire <- drinks %>% glm(formula = desired ~ nrel, data = .)
summary(mod_desire)
```

```{r}

model.neg.relationship <- glm(formula = numall ~ nrel + desired + nrel:desired, 
                              family = poisson(link="log"),
                              data = drinks)
summary(model.neg.relationship)
```

```{r}

mod_desired <- drinks %>% glm(formula = desired ~ nrel, data = .)
summary(mod_desired)
```
...
p2 <- drinks %>%
  ggplot(aes(numall, rosn)) +
  geom_boxplot(aes(fill = rosn)) +
  coord_flip() +
  ggtitle("GPA by Admission") +
  theme(plot.title = element_text(lineheight = 1, face = "bold")) +
  ylab("College GPA") +
  xlab("Admission")
...
#TODO: try classify the rosn into multiple categories (2 - 3), see if the groups vs base would have significant 
model.neg.relationship.rosn <- glm(formula = numall ~ nrel + rosn + nrel:rosn,  
                              family = poisson(link="log"),
                              data = drinks)
