...
```{r drinking EDA}
# limit the study to Saturday like the researchers
saturday <- drinks[drinks$dayweek == 6, c(1, 4, 5, 6, 10, 11)]
head(saturday)
pairs( saturday )
```
...
```{r}
model.neg.relationship <- glm(formula = numall ~ nrel, 
                              family = poisson(link="log"),
                              data = saturday)
summary(model.neg.relationship)
```
```{r}
# LRT on negative relationship along with confident interval
Anova(model.neg.relationship)
```
...
```{r}
# LRT on negative relationship along with confident interval
Anova(model.neg.relationship.rosn)
```
> 'Fill this in: What do you learn?' 
